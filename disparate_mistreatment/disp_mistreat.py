...
def get_clf_stats(w, x_train, y_train, x_control_train, x_test, y_test, x_control_test, sensitive_attrs):
    ...
    for s_attr in sensitive_attrs:
        ...
        cm = confusion_matrix(y_test, all_class_labels_assigned_test)
...
def get_sensitive_attr_constraint_fpr_fnr_cov(model, x_arr, y_arr_true, y_arr_dist_boundary, x_control_arr,
    ...
    for cons_type in [0, 1, 2]:
        ...
        if verbose == True:
            print('Covariance for type {} is: {:.7f}'.format(cov_type_name, cov))
...
def plot_fairness_acc_tradeoff(x_all, y_all, x_control_all, loss_function, cons_type):
    ...
    for c in cov_range:
        print('LOG: testing for multiplicative factor: {:.2f}'.format(c))
