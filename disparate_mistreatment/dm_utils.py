...
import dm_loss_funcs as lf  # our implementation of loss funcs
...
def print_classifier_fairness_stats(acc_arr, correlation_dict_arr, cov_dict_arr, s_attr_name):
    ...
    print
    "Accuracy: %0.2f" % (np.mean(acc_arr))
    print
    "Protected/non-protected in +ve class: %0.0f%% / %0.0f%%" % (prot_pos, non_prot_pos)
    print
    "P-rule achieved: %0.0f%%" % (p_rule)
    print
    "Covariance between sensitive feature and decision from distance boundary : %0.3f" % (
...
def compute_p_rule(x_control, class_labels):
    ...
    print
    print
    "Total data points: %d" % (len(x_control))
    print
    "# non-protected examples: %d" % (non_prot_all)
    print
    "# protected examples: %d" % (prot_all)
    print
    "Non-protected in positive class: %d (%0.0f%%)" % (non_prot_pos, non_prot_pos * 100.0 / non_prot_all)
    print
    "Protected in positive class: %d (%0.0f%%)" % (prot_pos, prot_pos * 100.0 / prot_all)
    print
    "P-rule is: %0.0f%%" % (p_rule)
    return p_rule
...
def get_one_hot_encoding(in_arr):
    ...
    if in_arr.dtype != 'float64' and in_arr.dtype != 'int' and in_arr.dtype != 'int64':
        raise ValueError("************* ERROR: Input arr does not have integer types")
        return None
...
def plot_cov_thresh_vs_acc_pos_ratio(x_all, y_all, x_control_all, num_folds, loss_function, apply_fairness_constraints,
    ...
    for c in cov_range:
        print
        "LOG: testing for multiplicative factor: %0.2f" % c
