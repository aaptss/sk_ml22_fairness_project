...
def test_compas_data():
    ...
    # X, y, sensitive_attrs_idx, _, x_control = bank.load_bank()
    sensitive_attrs = [i for i in x_control.keys()]
    ...
    def train_test_classifier():
        w = dm.train_model_disp_mist(x_train, y_train, x_control_train, loss_function, EPS, cons_params)

        train_score, test_score, cov_all_train, cov_all_test, s_attr_to_fp_fn_train, s_attr_to_fp_fn_test = \
            dm.get_clf_stats(w, x_train, y_train, x_control_train, x_test, y_test, x_control_test, sensitive_attrs)
    ...
    print("== Constraints on FPR ==")  # setting parameter for constraints
    cons_type = 4  # FPR constraint -- just change the cons_type, the rest of parameters should stay the same
    tau = 5.0
    mu = 1.2
    sensitive_attrs_to_cov_thresh = {"race":
                                         {0: {0: 0, 1: 0},
                                          1: {0: 0, 1: 0},
                                          2: {0: 0, 1: 0}}}  # zero covariance threshold, means try to get the fairest solution
