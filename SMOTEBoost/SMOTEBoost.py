...
class SMOTEBoost(BaseWeightBoosting, ClassifierMixin):
    ...
    def _boost_discrete(self, iboost, X, y, sample_weight, random_state):
        ...
        if self.storePerfomance:
            if iboost != 0:
                ...
                test_scores['TPR prot'] = tpr_p
                test_scores['TNR non-prot'] = tnr_np
                test_scores['TPR non-prot'] = tpr_np
                self.performance.append({'train_scores': train_scores, 'test_scores': test_scores})
    ...
    def calculate_fairness(self, data, labels, predictions):
        ...
        return abs((tpr_non_protected - tpr_protected)) + abs((tnr_non_protected - tnr_protected)), tpr_protected, tnr_protected, tpr_non_protected, tnr_non_protected
